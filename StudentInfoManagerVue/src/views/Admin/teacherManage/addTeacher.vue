<template>
  <div  style="display: flex;flex-direction: column;align-items: center; ">
      <div style="color: black;font-size: 30px;font-weight: bolder;margin-bottom: 30px;">Add Teacher</div>
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          if (that.ruleForm.tname === 'admin') {
            that.$message({
              showClose: true,
              message: 'The admin account cannot be added',
              type: 'error'
            });
            return
          }
          axios.post("http://localhost:9451/teacher/addTeacher", this.ruleForm).then(function (resp) {
            if (resp.data === true) {
              that.$message({
                showClose: true,
